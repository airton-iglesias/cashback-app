    ...
    return (
        <View className="flex-row gap-4">
            {options.map((opt: any, index: any) => (
                <View key={index}>
                    <View style={{width: 110,height: 162}} className={`${selected === index ? '' : 'hidden'} absolute border-2 rounded-xl border border-gray-400 opacity-20' rounded-lg z-10`}></View>
                    <TouchableOpacity
                    ...
                    >
                        {index === 0 ? <StoreIcon /> : <CalendarIconRadio/>}
