...
export default function SignupStep3({ route }: any) {
    ...
    useEffect(() => {
        if (pin.length === 6) {
            signupNavigation.navigate('SignupStep4', {email, password, country, currency, imageProfile, pin});
        }
    ...
    return (
        <SafeAreaView className="flex-1 bg-white">
            <View className="flex-1 items-center top-24">
                <Text className={"text-5xl font-bold mb-8"}>Criar pin</Text>
                <View className="w-full h-48 flex justify-center items-center">
                    <View className="flex-row justify-center items-center">
                        {Array(6).fill(0).map((_, index) => (
                            <Text key={index} className="text-5xl font-black mx-3">{pin[index] ? pin[index] : <Text className="text-[#848484]">•</Text>}</Text>
                        ))}
                    </View>
                </View>
